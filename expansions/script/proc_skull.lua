...
function Auxiliary.SkullFilter1(c,e,tp,f1,f2,skc)
	...
	return mic<=0 or rg:CheckSubGroup(Auxiliary.SkullFunction1,1,maxc,tp,mic,mac,skc)
end
...
function Auxiliary.SkullFunction1(g,tp,mic,mac,skc)
	local chkf=false
	for i=mic,mac do
	...
	return
		function(e,c,ischain,re,rp)
			...
			return c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_SKULL,tp,false,false)
				and Duel.IsExistingMatchingCard(Auxiliary.SkullFilter1,tp,LOCATION_HAND,0,1,nil,e,tp,f1,f2,c)
		end
end
function Auxiliary.SkullOperation(f1,f2,gf)
	return
		function(e,tp,eg,ep,ev,re,r,rp,c,sg,ischain)
			...
			local g=Duel.SelectMatchingCard(tp,Auxiliary.SkullFilter1,tp,LOCATION_HAND,0,0,1,nil,e,tp,f1,f2,c)
			local tc=g:GetFirst()
			if not tc then
				return
			end
			...
			local tg=rg:SelectSubGroup(tp,Auxiliary.SkullFunction1,true,0,maxc,tp,mic,mac,c)
