...
function s.tfil6(c,e)
	return c:IsNegatable() and c:IsCanBeEffectTarget(e)
end
function s.tfun6(sg,c,tp)
	return sg:IsExists(Card.IsControler,1,c,tp)
end
function s.tar6(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local c=e:GetHandler()
	local g=Duel.GMGroup(s.tfil6,tp,"O","O",nil,e)
	if chkc then
		return false
	end
	if chk==0 then
		return g:CheckSubGroup(s.tfun6,2,2,c,tp)
	end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	local sg=g:SelectSubGroup(tp,s.tfun6,false,2,2,c,tp)
