--RUM(랭크 업 매직)-헤븐리 트윈스 포스
local s,id=GetID()
function s.initial_effect(c)
	--자신의 패 / 묘지에서 "세레나데" 몬스터 1장을 고르고 수비 표시로 특수 소환한다.
	...
	c:RegisterEffect(e1)
	--그 몬스터보다 랭크가 2개 높은 "세레나데" 몬스터 1장을, 대상의 자신 몬스터의 위에 겹쳐 엑시즈 소환으로 취급하고 엑스트라 덱에서 특수 소환한다.
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0xc22) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
...
function s.filter1(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and c:IsFaceup() and c:IsSetCard(0xc22) and (rk>0 or c:IsStatus(STATUS_NO_LEVEL))
...
function s.filter2(c,e,tp,mc,rk,pg)
	...
	return mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsRank(rk) and c:IsSetCard(0xc22) and mc:IsCanBeXyzMaterial(c,tp)
